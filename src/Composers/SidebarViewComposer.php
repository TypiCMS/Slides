...
class SidebarViewComposer
{
    public function compose(View $view): void
    {
        ...
        $view->offsetGet('sidebar')->group(__(config('typicms.modules.slides.sidebar.group', 'Content')), function (SidebarGroup $group): void {
            $group->id = 'content';
            $group->weight = 30;
            $group->addItem(__(config('typicms.modules.slides.sidebar.label', 'Slides')), function (SidebarItem $item): void {
                $item->id = 'slides';
